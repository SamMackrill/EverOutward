...
4. Refresh the local app to see each home's saved totals and updated next five. Publishing is a separate action and uses these records. Publish the website after significant verified changes under the owner's standing instruction; keep plans local.

To deliberately replace **all** saved distances for the same home (for example after road or catalogue-coordinate changes), run `npm run routes:refresh -- --home HOME_ID --force`. Combine `--all-homes --force` to refresh them all. This also replaces manually entered distances when a new valid route is returned. Failed lookups keep previously saved distances; the report identifies those failures. Regular runs reuse saved distances regardless of age. Keep reports, backups and the database private and out of Git.
