...
Open **http://127.0.0.1:5173**. Choose **Owner sign-in** and create a password of at least 12 characters. The first journey is already configured privately on this computer from the supplied postcode; a fresh checkout asks you to set home in Workspace. The owner API listens only on `127.0.0.1:3001`.
...
Use **Workspace → Calculate free driving distances**, `npm run routes:refresh`, or publish the journal. Results are cached locally for seven days and tied to the home version. Requests identify the app, are spaced at least 1.1 seconds apart, and batch nearby destinations; public page views never call the routing server. Your starting coordinates are sent to FOSSGIS for calculation. Its community service has no availability guarantee; saved estimates remain available when it fails.

The calculation starts with nearby catalogue points. Once five road distances are known, it stops when more distant points cannot beat the fifth distance even after allowing up to 1 km of road snapping at each endpoint. Unknown nearby routes keep the queue provisional. Points more than 1 km from a routed road require entrance review. The displayed routes remain estimates to catalogue points; check the actual visitor entrance in Waze. Changing home invalidates saved estimates.

Manual distance entry and route JSON import are also available:

```json
[{"placeId":"ID from public/data/places.json","metres":12000,"seconds":900}]
```
